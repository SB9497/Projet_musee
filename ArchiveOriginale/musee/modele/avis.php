...
class  dao_avis
{
    public static function getavisbyoeuvre($id)
    {
        try {
            ...
            while ($ligne) {
                $resultat[] = new avis($ligne["id_utilisateur"], $ligne["id_oeuvre"], $ligne["commentaire"]);
                $ligne = $req->fetch(PDO::FETCH_ASSOC);
            }
    ...
    public static function addavis($id_utilisateur, $id_oeuvre, $avis, $date)
    {
        try {
            $cnx = connexionPDO();
            ...
            $req->bindValue(':id_oeuvre', $id_oeuvre, PDO::PARAM_INT);
